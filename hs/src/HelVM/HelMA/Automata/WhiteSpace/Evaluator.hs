...
batchSimpleEvalTL = flipSimpleEvalTL emptyInput
...
evalParams p = eval (source p) (asciiLabel p) (stack $ typeOptions p) (ram $ typeOptions p)
...
instance Evaluator Interact where
  ...
  doInputChar iu stack h (char:input) = next iu stack' (store (address::Symbol) (toInteger (ord char)) h) input
...
instance WrapperIO m => Evaluator (m ()) where
  ...
  doInputChar iu stack h = do
    char <- wGetChar
    next iu stack' (store (address::Symbol) (toInteger (ord char)) h)
      where (address, stack') = pop1 stack
